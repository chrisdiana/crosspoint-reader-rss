...
class App {
 public:
  ...
  App(NameGetter nameGetter, UIIcon icon, Factory factory, VisibleCallback visibleCallback = nullptr)
      : nameGetter(std::move(nameGetter)), icon(icon), factory(std::move(factory)), visibleCallback(std::move(visibleCallback)) {}

  App(std::string name, UIIcon icon, Factory factory)
      : nameGetter([n = std::move(name)]() { return n; }), icon(icon), factory(std::move(factory)), visibleCallback(nullptr) {}
